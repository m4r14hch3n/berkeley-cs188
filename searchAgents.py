...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state: Any):
        ...
        """
        while len(state[1]) == 1 and state[0] == state[1][0]:
            return True
        return False
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    #     max_distance = 0
    #     distance_temp = util.manhattanDistance(food_position, position)
    ...
    nearest_food_distance = min(distances_to_food)

    food_distances = []
    for food1 in food_positions:
        for food2 in food_positions:
            distance = util.manhattanDistance(food1, food2)
            food_distances.append(distance)

    max_food_distance = max(food_distances)

    return nearest_food_distance + max_food_distance
